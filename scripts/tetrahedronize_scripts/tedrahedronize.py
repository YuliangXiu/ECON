...
for sex in ["male", "female", "neutral"]:
    for age in ["kid", "adult"]:
        ...
        if True:
            ...
            tet_smpl_vertices = np.load(
                f"./data/tetgen_{sex}_{age}_vertices.npy")
            ...
            posed_shape_added = tet_smpl_vertices_added - smpl.trans.reshape(
                [1, 3])
            ...
            smpl = TetraSMPLModel(
                f"./smpl/SMPL_{sex.upper()}.pkl",
                f"./data/tetra_{sex}_{age}_smpl.npz",
                age,
            )

            smpl.set_params(beta=beta, pose=pose, trans=trans)
            smpl.save_tetrahedron_to_obj(
                f"./data/objs/tetrahedron_{sex}_{age}.obj")

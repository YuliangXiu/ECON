...
class BNI:
    ...
    def extract_surface(self, verbose=True):
        ...
        F_B_faces = torch.cat((bni_result["F_faces"], bni_result["B_faces"] + bni_result["F_faces"].max() + 1), dim=0)

        self.F_B_trimesh = trimesh.Trimesh(F_B_verts.float(), F_B_faces.long(), process=False, maintain_order=True)
...
if __name__ == "__main__":
    ...
    bni_object = BNI(osp.dirname(npy_file), osp.basename(npy_file), bni_dict, default_cfg, torch.device('cuda:0'))

...
def get_affine_matrix_box(boxes, w2, h2):
    ...
    center = torch.tensor([(boxes[:, 0] + boxes[:, 2]) / 2.0, (boxes[:, 1] + boxes[:, 3]) / 2.0]).T    #(N,2)
    scale = torch.min(torch.tensor([w2 / width, h2 / height]), dim=0)[0].unsqueeze(1).repeat(1, 2) * 0.9    #(N,2)
...
def process_image(img_file, hps_type, single, input_res, detector):
    ...
    img_square = warp_affine(
        img_raw, M_square[:, :2], (tgt_res, ) * 2, mode='bilinear', padding_mode='zeros', align_corners=True
    )
    ...
    for idx in range(len(boxes)):
        ...
        img_square_rgba = torch.cat(
            [img_square.squeeze(0).permute(1, 2, 0),
             torch.tensor(mask_detection < 0.4) * 255], dim=2
        )

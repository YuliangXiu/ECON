...
class TestDataset:
    ...
    def compute_voxel_verts(self, body_pose, global_orient, betas, trans,
        ...
        faces = (np.loadtxt(
            osp.join(self.smpl_data.tedra_dir,
    ...
    def __getitem__(self, index):
        ...
        arr_dict.update({
            ...
            img_name,
            "image":
            arr_dict["img_icon"].to(self.device).unsqueeze(0),
        })
        ...
        if self.hps_type == "pymaf":
            ...
            arr_dict["betas"] = (torch.from_numpy(
        ...
        arr_dict["trans"] = (torch.tensor([tranX, tranY, 0.0]).to(
        ...
        N_body = arr_dict["body_pose"].shape[1]
        arr_dict["body_pose"] = arr_dict["body_pose"][:, :, :, :2].reshape(
            1, N_body, -1)
        arr_dict["global_orient"] = arr_dict[

...
class PointFeat:
    def __init__(self, verts, faces):
        ...
        if verts.shape[1] == 10475:
            ...
            mouth_faces = (
                torch.as_tensor(SMPLX().smplx_mouth_fid).unsqueeze(0).repeat(self.Bsize, 1, 1).to(self.device)
            )
    ...
    def query(self, points):
        ...
        closest_triangles = torch.gather(self.triangles, 1, pts_ind[None, :, None, None].expand(-1, -1, 3,
                                                                                                3)).view(-1, 3, 3)
        ...
        closest_normals = torch.gather(feat_normals, 1, pts_ind[None, :, None, None].expand(-1, -1, 3,
                                                                                            3)).view(-1, 3, 3)

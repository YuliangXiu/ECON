...
class VoxelizationFunction(Function):
    ...
    def forward(
        ...
        semantic_volume = torch.cuda.FloatTensor(ctx.batch_size, ctx.volume_res, ctx.volume_res, ctx.volume_res,
                                                 3).fill_(0.0)
...
class Voxelization(nn.Module):
    ...
    def vertices_to_faces(self, vertices):
        ...
        face = (
            self.smpl_face_indices_batch + (torch.arange(bs, dtype=torch.int32).to(self.device) * nv)[:, None, None]
        )

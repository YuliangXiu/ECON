...
class NormalNet(BasePIFuNet):
    ...
    def __init__(self, cfg):
        ...
        self.in_nmlF = [item[0] for item in self.opt.in_nml if "_F" in item[0] or item[0] == "image"]
        self.in_nmlB = [item[0] for item in self.opt.in_nml if "_B" in item[0] or item[0] == "image"]
        self.in_nmlF_dim = sum([item[1] for item in self.opt.in_nml if "_F" in item[0] or item[0] == "image"])
        self.in_nmlB_dim = sum([item[1] for item in self.opt.in_nml if "_B" in item[0] or item[0] == "image"])
    ...
    def get_norm_error(self, prd_F, prd_B, tgt):
        ...
        if 'gan' in self.ALL_losses:
            ...
            if 'gan_feat' in self.ALL_losses:
                ...
                total_loss["G_GAN_Feat"] = loss_G_GAN_Feat * self.B_losses_ratio[self.B_losses.index('gan_feat')]

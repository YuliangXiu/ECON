...
def augment_bbox(center, bbox_size, scale=[1.0, 1.0], trans_scale=0.):
    ...
    trans_scale = (torch.rand([batch_size, 2],
                   device=center.device)*2. - 1.) * trans_scale
...
def crop_tensor(image, center, bbox_size, crop_size, interpolation='bilinear', align_corners=False):
    ...
    src_pts = torch.zeros([4, 2], dtype=dtype, device=device).unsqueeze(
...
class Cropper(object):
    ...
    def crop(self, image, points, points_scale=None):
        ...
        # crop
        cropped_image, tform = crop_tensor(
    ...
    def transform_points(self, points, tform, points_scale=None, normalize=True):
        ...
        trans_points_2d = torch.bmm(
            torch.cat([points_2d, torch.ones([batch_size, n_points, 1],
                      device=points.device, dtype=points.dtype)], dim=-1),
            tform
        )
        trans_points = torch.cat(
            [trans_points_2d[:, :, :2], points[:, :, 2:]], dim=-1)
        if normalize:

...
class SRenderY(nn.Module):
        ...
        if lights is not None:
            if lights.shape[1] == 9:
                ...
                if light_type == "point":
                    ...
                    shading_images = shading.reshape([batch_size, albedo_images.shape[2], albedo_images.shape[3],
                                                      3]).permute(0, 3, 1, 2)
                else:
    ...
    def render_shape(
        ...
        shading_images = (
            shading.reshape([batch_size, albedo_images.shape[2], albedo_images.shape[3], 3]).permute(0, 3, 1,
                                                                                                     2).contiguous()
        )

...
class PIXIE(object):
    ...
    def encode(
        ...
        for key in data.keys():
            ...
            if key == "body":
                ...
                if (
                    "head_image" not in data[key].keys() or "left_hand_image" not in data[key].keys() or
                ...
                for part_name in ["head", "left_hand", "right_hand"]:
                    ...
                    if copy_and_paste:
                        ...
                        f_body_out = (feature["body"][f"{part_name}_share"] * (1.0 - part_w) + f_part * part_w)
    ...
    def convert_pose(self, param_dict, param_type):
        ...
        if param_type == "head":
            ...
            param_dict["neck_pose"] = self.smplx.neck_pose.unsqueeze(0).expand(batch_size, -1, -1, -1)
            param_dict["left_wrist_pose"] = self.smplx.neck_pose.unsqueeze(0).expand(batch_size, -1, -1, -1)
            param_dict["left_hand_pose"] = self.smplx.left_hand_pose.unsqueeze(0).expand(batch_size, -1, -1, -1)
            param_dict["right_wrist_pose"] = self.smplx.neck_pose.unsqueeze(0).expand(batch_size, -1, -1, -1)
            param_dict["right_hand_pose"] = self.smplx.right_hand_pose.unsqueeze(0).expand(batch_size, -1, -1, -1)
            ...
            param_dict["head_pose"] = self.smplx.head_pose.unsqueeze(0).expand(batch_size, -1, -1, -1)
            param_dict["neck_pose"] = self.smplx.neck_pose.unsqueeze(0).expand(batch_size, -1, -1, -1)

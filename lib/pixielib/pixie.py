...
class PIXIE(object):
    ...
    def convert_pose(self, param_dict, param_type):
        ...
        if param_type == 'head':
            ...
            x_180_pose = torch.eye(3, dtype=dtype,
                                   device=device).unsqueeze(0).repeat(1, 1, 1)
            x_180_pose[0, 2, 2] = -1.
            x_180_pose[0, 1, 1] = -1.
    ...
    def decode(self, param_dict, param_type):
        ...
        param_dict['body_pose'] = torch.cat([
            partbody_pose[:, :11], param_dict['neck_pose'],
            partbody_pose[:, 11:11 + 2], param_dict['head_pose'],
            partbody_pose[:, 13:13 + 4], param_dict['left_wrist_pose'],
    ...
    def decode_Tpose(self, param_dict):
        ...
        '''
        verts, _, _ = self.smplx(shape_params=param_dict['shape'],
                                 expression_params=param_dict['exp'],
                                 jaw_pose=param_dict['jaw_pose'])
        return verts

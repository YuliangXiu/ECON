...
from pytorch3d.renderer import (
    ...
    RasterizationSettings, BlendParams, MeshRenderer, MeshRasterizer, SoftPhongShader, SoftSilhouetteShader,
    HardPhongShader, HardGouraudShader, HardFlatShader, TexturesVertex
)
...
class OpenDRenderer:
    def __init__(self, resolution=(224, 224), ratio=1):
        ...
        self.K = np.array(
            [
                [self.focal_length, 0., self.resolution[1] / 2.], [0., self.focal_length, self.resolution[0] / 2.],
                [0., 0., 1.]
            ]
        )
        self.colors_dict = {
...
class IUV_Renderer(object):
    def __init__(
        ...
        if mode in ['iuv']:
            ...
            with open(
                os.path.join(path_config.SMPL_MODEL_DIR, '{}_vert_segmentation.json'.format(body_model)), 'rb'
            ) as json_file:
        ...
        K = np.array(
            [[self.focal_length, 0., self.orig_size / 2.], [0., self.focal_length, self.orig_size / 2.], [0., 0., 1.]]
        )
    ...
    def verts2iuvimg(self, verts, cam, iwp_mode=True):
        ...
        cameras = PerspectiveCameras(
            device=verts.device, R=R, T=t, K=K, in_ndc=False, image_size=[(self.orig_size, self.orig_size)]
        )

...
def sample_patch_points(batch_size, n_points, patch_size=1, image_resolution=(128, 128), continuous=True):
    ...
    if continuous:
        ...
        px = torch.randint(0, image_resolution[1], size=(batch_size, n_patches, 1)).float() / (image_resolution[1] - 1)
        py = torch.randint(0, image_resolution[0], size=(batch_size, n_patches, 1)).float() / (image_resolution[0] - 1)
...
def check_ray_intersection_with_unit_cube(ray0, ray_direction, padding=0.1, eps=1e-6, scale=2.0):
    ...
    p_mask_inside_cube = (
        (p_intersect[:, :, :, 0] <= p_distance + eps) & (p_intersect[:, :, :, 1] <= p_distance + eps) &
        (p_intersect[:, :, :, 2] <= p_distance + eps) & (p_intersect[:, :, :, 0] >= -(p_distance + eps)) &
        (p_intersect[:, :, :, 1] >= -(p_distance + eps)) & (p_intersect[:, :, :, 2] >= -(p_distance + eps))
    ).cpu()
...
def intersect_camera_rays_with_unit_cube(
    ...
    if not use_ray_length_as_depth:
        p_cam = transform_to_camera_space(p_cube.view(batch_size, -1, 3), camera_mat, world_mat,
                                          scale_mat).view(batch_size, n_points, -1, 3)
        d_cube = p_cam[:, :, :, -1]
...
def transform_points_batch(p, transform):
    ...
    pcl_hom = torch.cat([p, torch.ones(p.shape[0], p.shape[1], 1).to(device)], dim=-1).transpose(2, 1)
...
def transform_to_camera_space(p_world, camera_mat, world_mat, scale_mat):
    ...
    p_world = torch.cat([p_world, torch.ones(batch_size, n_p, 1).to(device)], dim=-1).permute(0, 2, 1)
...
def get_occupancy_loss_points(
    ...
    if depth_image is not None:
        depth_gt, mask_gt_depth = get_tensor_values(depth_image, pixels, squeeze_channel_dim=True, with_mask=True)
        d_occupancy[mask_gt_depth] = depth_gt[mask_gt_depth]

    p_occupancy = transform_to_world(pixels, d_occupancy.unsqueeze(-1), camera_mat, world_mat, scale_mat)
    return p_occupancy
...
def get_freespace_loss_points(
    ...
    p_freespace = transform_to_world(pixels, d_freespace.unsqueeze(-1), camera_mat, world_mat, scale_mat)
    return p_freespace

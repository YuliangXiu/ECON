                ...
                if j > i:
                    fuse_layer.append(
                        nn.Sequential(
                            ...
                            nn.BatchNorm2d(num_inchannels[i]), nn.Upsample(scale_factor=2**(j - i), mode='nearest')
                        )
                    )
                elif j == i:
        ...
        for i in range(num_branches_cur):
            if i < num_branches_pre:
                ...
                for j in range(i + 1 - num_branches_pre):
                    ...
                    conv3x3s.append(
                        nn.Sequential(
                            nn.Conv2d(inchannels, outchannels, 3, 2, 1, bias=False), nn.BatchNorm2d(outchannels),

...
def batch_inverse_kinematics_transform(
    ...
    final_pose_skeleton = (final_pose_skeleton - final_pose_skeleton[:, 0:1] +
                           rel_rest_pose[:, 0:1])
            ...
            rotate_rest_pose[:,
                             i] = rotate_rest_pose[:,
                                                   parents[i]] + torch.matmul(
                                                       ...
                                                       rel_rest_pose[:, i])
            # (B, 3, 1)
            child_final_loc = (final_pose_skeleton[:, children[i]] -
                               rotate_rest_pose[:, i])
...
def batch_inverse_kinematics_transform_optimized(
    ...
    # the predicted final pose
    final_pose_skeleton = torch.unsqueeze(pose_skeleton.clone(), dim=-1)
    final_pose_skeleton = (final_pose_skeleton - final_pose_skeleton[:, [0]] +
                           rel_rest_pose[:, [0]])
        ...
        if idx_lev == len(idx_levs) - 1:
            ...
            rotate_rest_pose[:, indices] = rotate_rest_pose[:, parents[
                indices]] + torch.matmul(rot_mat_chain[:, parents[indices]],
                                         rel_rest_pose[:, indices])
            # (B, 3, 1)
            child_final_loc = (final_pose_skeleton[:, children[indices]] -
                               rotate_rest_pose[:, indices])
            ...
            K = torch.cat([zeros, -rz, ry, rz, zeros, -rx, -ry, rx, zeros],
                          dim=2).view((batch_size, len_indices, 3, 3))
...
def vectors2rotmat(vec_rest, vec_final, dtype):
    ...
    # (B, 1, 1)
    cos = torch.sum(vec_rest * vec_final, dim=1,
                    keepdim=True) / (vec_rest_norm * vec_final_norm + 1e-8)

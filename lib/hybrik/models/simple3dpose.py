    ...
    def forward(
        ...
        **kwargs,
        ...
        pred_xyz_jts_29[:, :, 2:] = pred_uvd_jts_29[:, :,
                                                    2:].clone()  # unit: 2.2m
        pred_xyz_jts_29_meter = (
            pred_uvd_jts_29[:, :, :2] * self.input_size / self.focal_length
        ...
        transl = pred_xyz_jts_29[:, 0, :] * 2.2 - pred_xyz_jts_17[:,
                                                                  0, :] * 2.2
        transl[:, :2] += camTrans[:, 0]
        ...
        new_cam[:, 1:] = transl[:, :2]
        new_cam[:, 0] = self.focal_length / (self.input_size * transl[:, 2] +
                                             1e-9)
